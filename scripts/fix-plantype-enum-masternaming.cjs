...
// master_naming(마스터작명 30만원)을 추가한다.
//
// 사용법 (Railway 콘솔):
//   node scripts/fix-plantype-enum-masternaming.cjs
...
(async () => {
  ...
  console.log("[1/3] payments.planType 에 master_naming 추가...");
  await c.execute(`
    ALTER TABLE payments MODIFY COLUMN planType ENUM(
      ...
      'self_naming','master_naming'
    ) NOT NULL
  `);

  console.log("[2/3] consultSessions.planType 에 master_naming 추가...");
  await c.execute(`
  ...
  console.log("\n완료. 마스터작명 결제(무통장입금/카드결제) 모두 정상 작동해야 합니다.");
  await c.end();
})().catch((e) => {
  console.error("실패:", e);
  process.exit(1);
